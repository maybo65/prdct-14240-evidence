...
| 13 | `DefaultHookTimeoutSeconds` (`scanner/internal/agents/kimi/constants.go:41`) | (new constant) | `30` (seconds) | **Every endpoint where `kimi-hooks-install` runs.** Written as the `timeout` on each of the three Onyx hook entries (`kimi_hooks_installer.go:240-242`) and into `onyx-config.json` (`:444`). On a **fresh** install the three config.toml hook entries carry `timeout=30`; on a re-run of an **already-configured** host the installer takes the idempotent "refreshing sidecar only" path (`:148/:152`) — it rewrites `onyx-config.json` (so its `Timeout` becomes 30) and refreshes the binary, but leaves the existing config.toml hook-table entries untouched, so a prior timeout is preserved unless `--force` rewrites them. Mirrors Kimi's own `HookDef.timeout` default of 30. | N (per-install operational default; not customer-data-visible, matches Kimi's own default) |
| 14 | `PreToolUseMatcher` (`scanner/internal/agents/kimi/constants.go:64`) | (new constant) | `""` (empty = match-all) | **Every endpoint where hooks install.** The `matcher` field on the single Onyx PreToolUse entry AND the single PostToolUse entry. Empty = match-all, so one entry intercepts/audits every tool (built-in shell + any MCP tool) and cannot silently miss a renamed/new tool. | N — **fixed design constant, not a tunable default**: it encodes the interception *scope* (match-all) and is intentionally not user-overridable; documented here for completeness because it is written into every install. |

Rows 1–4 and 12 are one coupled owner decision (revive + fleet-wide enablement + firing scope + compliance mapping + the mirrored Go risk score). Rows 5–7 are one display-fallback change across three reads. Rows 8–11 are internal/CLI defaults. Rows 13–14 are per-install hook-installer constants (13 is an operational timeout matching Kimi's own default; 14 is a fixed match-all scope, not a tunable).

### Re-sweep — every other added value that could be a default (the board's second ask)
I re-walked the full `git diff origin/main...HEAD` for any value that takes effect when nobody chose one. Findings, each resolved:
- **`FULL_CAP_ASSET_IDS = 1000`** (`test_issue_detection_payload_bounds.py`) — a **test** constant that mirrors `AGENT_ISSUE_RECALC_BATCH_SIZE` (row 8) to exercise the payload cap. Not a runtime/customer default; not inventoried as one.
- **Pydantic `Field(default=None)` / `Field(default="")`** on the Kimi hook payload models (`kimi_parser.py:1243-1278` region — `timestamp`, `session_id`, `hook_event_name`, `cwd`, `prompt`, `tool_name`, `tool_call_id`, `tool_output`) — **parse-model field defaults**: an absent field on the incoming hook JSON deserializes to None/empty. These are input-shape defaults for a parser, not behavioral/product defaults that change what a customer gets, and there is no owner decision in "a missing optional wire field is None". Stated explicitly as **not** owner-decision defaults.
- **`json.dumps(..., default=str)`** (`:1403/:1430`) — the stdlib serializer *fallback function* argument, not a default value.
- **`dsl_capability_ops.go`** — its additions are capability **declarations** (what the agent can do), not values that take effect when unset; `git diff` shows no `default`/scope assignment there. Stated explicitly as **no defaults added**.

Conclusion of the re-sweep: the only runtime default not previously inventoried is `DefaultHookTimeoutSeconds = 30` (now row 13); `PreToolUseMatcher = ""` is a fixed design constant (row 14); nothing else in the diff introduces an uninventoried default.
